  ...
  const tableData = useMemo(() => {
    ...
    return sessions.map((session, index) => ({
      ...
      date: session.dateCreated 
        ? new Date(session.dateCreated).toLocaleDateString("vi-VN")
        : "N/A",
      status: session.status,
      startTime: session.startedTime
        ? new Date(session.startedTime).toLocaleTimeString("vi-VN")
        : "N/A",
      endTime: session.endedTime
        ? new Date(session.endedTime).toLocaleTimeString("vi-VN")
        : null,
      kwh: session.kwh || 0,
