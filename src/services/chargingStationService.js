...
export const chargingStationService = {
  ...
  async getAllStations() {
    try {
      ...
      // Trả về dữ liệu trực tiếp từ API
  ...
  async getStationPosts(stationId) {
    try {
      ...
      const response = await apiClient.get(`/charging/station/posts/${stationId}`);
...
export const stationDataMapper = {
  ...
  calculateStatistics(stations) {
    ...
    // Tính tổng cổng sạc
    const totalPosts = stations.reduce((sum, station) => sum + (station.numberOfPosts || 0), 0);
    const availablePosts = stations.reduce((sum, station) => sum + (station.availableSlots || 0), 0);
    ...
    return {
      ...
      averagePostsPerStation: totalStations > 0 ? Math.round(totalPosts / totalStations) : 0,
    };
  },
  ...
  mapPostFromApi(apiPost) {
    return {
      ...
      isAvailable: apiPost.isActive && !this.isPostBusy(apiPost.chargingSessions),
      supportedTypes: apiPost.chargingType?.map(type => type.typeName || type.name || 'AC') || ['AC'],
    };
  },
  ...
  isPostBusy(chargingSessions) {
    ...
    return chargingSessions.some(session => 
